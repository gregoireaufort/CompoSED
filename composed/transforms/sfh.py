...
def normalize_sfh_to_formed_mass(time_gyr: np.ndarray, sfr_msun_per_yr: np.ndarray) -> np.ndarray:
    ...
    if hasattr(np, "trapezoid"):
        formed_mass = np.trapezoid(sfr_msun_per_yr, time_gyr) * 1e9
    else:
        formed_mass = trapezoid(sfr_msun_per_yr, time_gyr) * 1e9
    if not np.isfinite(formed_mass) or formed_mass <= 0.0:
        raise ModelDomainError(f"Invalid formed mass from SFH integral: {formed_mass}")
